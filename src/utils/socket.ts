...
export function setupSocket(io: Server) {
    io.on("connection", async (socket: Socket) => {
        ...
        socket.on("joinMatchGame", async (data: JoinGame) => {
        ...
        socket.on("disconnect", async () => {
            ...
            if (disconnectedUser) {
                activeUsersMap.delete(disconnectedUser[0]); // ➖ Remove from map
                console.log(`User ${disconnectedUser[0]} removed from active users`);
            }
            await removePlayer(socket.id);
        });
    });
    ...
    try {
      ...
      if (opponent) {
        ...
        io.to(opponentSocketId).emit("opponentFound", {
          opponent_id: userId,
          opponent_name: currentUser?.name,
...
async function handleGameFinish(io: Server, socket: Socket, data: {
    ...
    try {
      ...
      if (game.playerId.toString() === userId) {
        ...
        if (game.isBotMatch) {
          ...
          if (bot) {
            ...
            return setTimeout(() => simulateBotPlay(io, roomId, userId, bot, score), 3000);
          }
          return;
        }
      }
      ...
      const [populatedGame, settings] = await Promise.all([
          GameResult.findById(game._id)
            .populate<{ playerId:IUser}>("playerId", "name avatarUrl referredBy")
            .populate<{ opponentId:IUser}>("opponentId", "name avatarUrl referredBy")
      ...
      if ((game.playerScore ?? 0) > (game.opponentScore ?? 0)) {
        ...
        winner = { id: player._id, name: player.name, avatarUrl: player.avatarUrl, score: game.playerScore };
        loser = { id: opponent._id, name: opponent.name, avatarUrl: opponent.avatarUrl, score: game.opponentScore };
      } else if ((game.playerScore ?? 0) < (game.opponentScore ?? 0)) {
        winnerUserId = opponent._id.toString();
        winner = { id: opponent._id, name: opponent.name, avatarUrl: opponent.avatarUrl, score: game.opponentScore };
        loser = { id: player._id, name: player.name, avatarUrl: player.avatarUrl, score: game.playerScore };
      }
      ...
      io.to(roomId).emit("gameResult", {
        winner: { ...winner, earn: netToWinner },
        loser: { ...loser, earn: 0, loseAmount: subGame.entry }
      });
  
    } catch (error: any) {
      console.error("❌ Error in handleGameFinish:", error.message || error);
...
const findMatchWithOpponent = async (payload: JoinGame): Promise<IOpponent | null> => {
    ...
    const opponent: any | null = await Matchmaking.findOne({
        ...
        status: "waiting",
        userId: { $ne: userId }
    }).populate<{ userId: IUser }>("userId", "name avatarUrl");
